...
func (vsl *virtualServiceLinker) getHostsByVirtualService(vs *kiali.VirtualService) []string {
	...
	http := vs.Spec["http"].([]interface{})
	for _, element := range http {
		elementMap := element.(map[string]interface{})
		if elementMap["route"] == nil {
			continue
		}
		route := elementMap["route"].([]interface{})
		for _, element2 := range route {
			element2Map := element2.(map[string]interface{})
			if element2Map["destination"] == nil {
				continue
			}
			destination := element2Map["destination"].(map[string]interface{})
			hosts = append(hosts, destination["host"].(string))
			logging.GetLogger().Errorf("tomer: %v", destination["host"].(string))
		}
	}
	return hosts
}
...
func (vsl *virtualServiceLinker) getAppsAndVersionsByDestinationRules(drs []*kiali.DestinationRule) ([]string, []string) {
	...
	for _, dr := range drs {
		...
		for _, drSubset := range subsets {
			subset := drSubset.(map[string]interface{})
			labels := subset["labels"].(map[string]interface{})
	...
	for _, dr := range vsl.drCache.List() {
		if dr, ok := dr.(*kiali.DestinationRule); ok {
			...
			for _, subset := range subsets {
				convsubset := subset.(map[string]interface{})
				labels := convsubset["labels"].(map[string]interface{})
...
func (vsl *virtualServiceLinker) getPodsByAppsAndVersions(apps, versions []string) []*v1.Pod {
	pods := []*v1.Pod{}
	for _, podObj := range vsl.podCache.List() {
		pod := podObj.(*v1.Pod)
		for i, _ := range apps {
